...
import { getUserColor, BOTH_COLOR, getStatusColor } from '../lib/colors'
...
export default function FutureView({ currentUser, users, calendarEnabled, onOpenMenu, onComplete }) {
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      const promises = [api.getTasks(from, toStr), api.getDayStatuses(from, toStr), api.getDailyScheduleEntries(from, toStr)]
      if (calendarEnabled) {
        promises.push(api.getCalendarEvents(from))
      }
      ...
      setDayStatuses(results[1] || {})
      setDailyEntries(results[2] || {})

      if (calendarEnabled && results[3]) {
        // Only keep events within our lookahead window
        const events = (results[3].events || []).filter(e => e.start_date <= toStr)
  ...
  // Add dates that have day statuses (even if no tasks/events)
  for (const dateStr of Object.keys(dayStatuses)) {
    if (dayStatuses[dateStr]?.length > 0 && !grouped[dateStr]) {
      grouped[dateStr] = { tasks: [], ghosts: [], events: [] }
    }
  }

  // Add dates that have daily schedule entries (even if no tasks/events)
  for (const dateStr of Object.keys(dailyEntries)) {
    if (dailyEntries[dateStr]?.length > 0 && !grouped[dateStr]) {
      grouped[dateStr] = { tasks: [], ghosts: [], events: [] }
    }
  }
      ...
      <div className="px-4 pt-3 space-y-4">
        {dates.length === 0 ? (
          ...
          dates.map(dateStr => {
            ...
            const oneOffTasks = activeTasks.filter(t => !t.schedule_id)
            const recurringTasks = activeTasks.filter(t => !!t.schedule_id)

            const hasDailyEntries = dailyEntries[dateStr]?.length > 0
            const hasStatuses = dayStatuses[dateStr]?.length > 0
            const hasContent = oneOffTasks.length > 0 || recurringTasks.length > 0 || day.ghosts.length > 0 || day.events.length > 0 || completedTasks.length > 0 || hasStatuses || hasDailyEntries
            ...
            return (
              <div key={dateStr}>
                ...
                <div className="space-y-1">
                  {/* Calendar events — prominent with distinct style */}
                  {day.events.map(event => (
                    <div key={`cal-${event.id}`} className="flex items-start gap-2.5 px-3 py-2 bg-pastel-sky/20 rounded-xl border border-pastel-sky/30">
